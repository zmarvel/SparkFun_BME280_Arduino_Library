...
uint8_t BME280::begin() {
  ...
  calibration.dig_H4 =
      ((int16_t)((readRegister(BME280_DIG_H4_MSB_REG) << 4) +
                 (readRegister(BME280_DIG_H4_LSB_REG) & 0x0F)));
  calibration.dig_H5 =
      ((int16_t)((readRegister(BME280_DIG_H5_MSB_REG) << 4) +
...
void BME280::setTempOverSample(uint8_t overSampleAmount) {
  ...
  uint8_t originalMode =
      getMode(); // Get the current mode so we can go back to it at the end

  setMode(MODE_SLEEP); // Config will only be writeable in sleep mode, so first
                       // go to sleep mode
...
void BME280::reset(void) { writeRegister(BME280_RST_REG, 0xB6); }
...
// Read all sensor registers as a burst. See BME280 Datasheet section 4. Data
...
float BME280::readFloatPressure(void) {
  ...
  int32_t adc_P = ((uint32_t)buffer[0] << 12) | ((uint32_t)buffer[1] << 4) |
                  ((buffer[2] >> 4) & 0x0F);
...
void BME280::readFloatPressureFromBurst(
    uint8_t buffer[], BME280_SensorMeasurements *measurements) {

  // Set pressure in Pa as unsigned 32 bit integer in Q24.8 format (24 integer
  ...
  if (var1 == 0) {
    ...
    p_acc = (((p_acc << 31) - var2) * 3125) / var1;
    var1 =
        (((int64_t)calibration.dig_P9) * (p_acc >> 13) * (p_acc >> 13)) >> 25;
    var2 = (((int64_t)calibration.dig_P8) * p_acc) >> 19;
    p_acc = ((p_acc + var1 + var2) >> 8) + (((int64_t)calibration.dig_P7) << 4);
...
float BME280::readFloatAltitudeMeters(void) {
  ...
  // Getting height from a pressure reading is called the "international
  // barometric height formula". The magic value of 44330.77 was adjusted in
  // issue #30. There's also some discussion of it here:
  // https://www.sparkfun.com/tutorials/253 This calculation is NOT designed to
  // work on non-Earthlike planets such as Mars or Venus; see NRLMSISE-00.
  // That's why it is the "international" formula, not "interplanetary".
...
void BME280::readFloatHumidityFromBurst(
    uint8_t buffer[], BME280_SensorMeasurements *measurements) {

  // Set humidity in %RH as unsigned 32 bit integer in Q22. 10 format (22
  // integer and 10 fractional bits). Output value of “47445” represents
  // 47445/1024 = 46. 333 %RH
...
float BME280::readTempC(void) {
  ...
  int32_t adc_T = ((uint32_t)buffer[0] << 12) | ((uint32_t)buffer[1] << 4) |
                  ((buffer[2] >> 4) & 0x0F);
...
float BME280::readTempFromBurst(uint8_t buffer[]) {
  int32_t adc_T = ((uint32_t)buffer[3] << 12) | ((uint32_t)buffer[4] << 4) |
                  ((buffer[5] >> 4) & 0x0F);
